...
displayUploadedFile <- function(data, input){
...
#FUNCTION: to make bezier curves
#Define function to plot bezier curves
makeBezierCurves <- function(data,input){
...
#FUNCTION: to plot bezier curves
plotBezierCurves <- function(data,input){
...
server <- function(input, output, session) {
  ...
  observeEvent(input$file1,{
    ...
    output$contents <- renderTable({
      displayUploadedFile(data,input)
    })
    ######
    
    ##### Define reactive function to plot cytoscape network
    ...
    observeEvent(input$submit, {
      ...
      output$bezierplot <- renderPlot({
        isolate({ plotBezierCurves(data,input) })
      })
